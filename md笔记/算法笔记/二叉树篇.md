...
##### 在递归前中后序的选择上，前序一般是求深度，后序一般是求高度。本题采用后序遍历
...
##### 本题求最小深度，还是用后序遍历，但是他并不像求最大深度那道题，这里最小深度指叶子结点的深度，如果单纯把`max`改成`min`求得就不是叶子结点
...
#### 对应力扣 [513. 找树左下角的值](https://leetcode.cn/problems/find-bottom-left-tree-value/)

##### 利用层序遍历能很好地解决这道题，每次读取这一层的第一个节点的值，直到最后一层，输入`result`
...
#### 对应力扣 [404. 左叶子之和](https://leetcode.cn/problems/sum-of-left-leaves/)

##### 本题的难点在于对于左叶子节点的定义，对于当前节点我们没法知道他是不是左节点，我们唯一能知道的是他是不是叶子结点，要判断他是不是左节点只能通过他的父节点判断，基于这点我们可以利用后序遍历
...
```c++
	int sumOfLeftLeaves(TreeNode* root) {
        ...
        //对于这个节点如果他拥有左节点，并且左节点是叶子结点就是我们要找的值
